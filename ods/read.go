...
func readContent(reader *zip.Reader, target calculus.Workbook) error {
	...
	if contentData != nil {
		if err := xml.Unmarshal(contentData, &content); err != nil {
			return errors.New(err, "Failed to parse shared strings table file")
		}
	}
	for _, table := range content.Body.Spreadsheet.Tables {
		sheet, err := target.AddSheet(table.Name)
		if err != nil {
			return errors.New(err, "Failed to create worksheet")
		}
		for rowIndex, row := range table.Rows {
			for cellIndex, cell := range row.Cells {
				...
				case "float", "currency", "percentage":
					...
					if err != nil {
						return errors.Newf(err, "Failed to parse value as number: \"%+v\"", cell.Value)
					}
				...
				case "date":
					...
					if err != nil {
						return errors.Newf(err, "Failed to parse value as date: \"%+v\"", cell.DateValue)
					}
					sheet.SetValue(rowIndex, cellIndex, t)
				case "time":
					d, err := duration.Parse(cell.TimeValue)
					if err != nil {
						return errors.Newf(err, "Failed to parse value as time: \"%+v\"", cell.TimeValue)
					}
